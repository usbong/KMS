...
class File extends CI_Controller { //MY_Controller {
...
*/					  
	...
	public function confirmStoreFileOnly($transactionId)
	{	
		if (isset($_SESSION['jsonResponses'])) {
			unset($_SESSION['jsonResponses']);
		...
		switch ($data["reportTypeId"]) {
			...
			case 9: //Report File
				...
				for($i=0;$i<$fileCount;$i++)
				{
					...
					$outputFolder = "files"; //note: this folder already exists
					//$outputFolder = "assets"; //note: this folder already exists	
...
//		if ($data["is_success"]) {									
		if (!empty($data["is_success"])) {									
			if (!empty($responses)) {
				...
				echo "<script>
						alert('You have successfully submitted your report. Thank you. Peace.');
						window.location.href='".site_url("report/storeReportFileOnly")."';
					  </script>";			
			}
			else { 
				echo "<script>
