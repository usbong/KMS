...
class Audio extends CI_Controller { //MY_Controller {
...
*/					  
	...
	public function confirmStoreAudioOnly($transactionId)
	{	
		if (isset($_SESSION['jsonResponses'])) {
			unset($_SESSION['jsonResponses']);
		...
		switch ($data["reportTypeId"]) {
			//TO-DO: -update: 3, 4.. with name constant 
			case 9: //Report Audio; //5: //Report Image
				$data["memberId"] = 1;
				$data["reportItemId"] = $count; //1
				...
				for($i=0;$i<$fileCount;$i++)
				{
					...
					$outputFolder = "sounds"; //note: this folder already exists
					//$outputFolder = "assets"; //note: this folder already exists	
					...
					$data["is_success"] = $this->Report_Model->insertReportAudio($data);
...
*/
				echo "<script>
						alert('You have successfully submitted your report. Thank you. Peace.');
						window.location.href='".site_url("report/storeReportAudioOnly")."';
					  </script>";			
