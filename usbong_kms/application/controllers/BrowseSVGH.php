...
class BrowseSVGH extends CI_Controller { //MY_Controller {
...
*/		
	...
	public function searchPatientSVGH()
	{		
		$data['param'] = $this->input->get('param'); //added by Mike, 20170616
		...
		$this->load->view('searchPatientSVGH', $data);
	}

	//added by Mike, 20200517; edited by Mike, 20200704
	public function confirmPatientSVGH()
	{
		//edited by Mike, 20200407
		$data['nameParam'] = $_POST['nameParam'];
		...
		//added by Mike, 20200328
		if (!isset($data['nameParam'])) {
			redirect('browseSVGH/searchPatientSVGH');
		}
		...
		//edited by Mike, 20200602
		//$data['result'] = $this->Browse_Model->getDetailsListViaName($data);
		$data['result'] = $this->BrowseSVGH_Model->getNewestPatientDetailsListViaName($data);
	...
	public function confirmPatientInformationDeskSVGH()
	{
		...
		//added by Mike, 20200328
		if (!isset($data['nameParam'])) {
			redirect('browseSVGH/searchPatientInformationDeskSVGH');
		}
...
*/		
	...
	//added by Mike, 20200628
	public function addNewTransactionForPatientSVGH($patientId, $medicalDoctorId)
	{		
		$data['patientId'] = $patientId;
		$data['medicalDoctorId'] = $medicalDoctorId;
		...
		$this->BrowseSVGH_Model->addNewTransactionForPatientPTRehab($data);
...
//		$data["result"] = $this->Report_Model->getPatientQueueReportForTheDay();
		...
		//$this->load->view('viewReportPatientQueue', $data);
//		redirect('report/viewReportPatientQueue');
		redirect('reportSVGH/viewReportPTRehabListOfAppointment');	
	}
