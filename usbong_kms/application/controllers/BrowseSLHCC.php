...
class BrowseSLHCC extends CI_Controller { //MY_Controller {
...
*/		
	...
	public function searchPatientPrev()
	{		
		$data['param'] = $this->input->get('param'); //added by Mike, 20170616
	...
	//added by Mike, 20200517
	public function confirmPatientPrev()
	{
		//edited by Mike, 20200407
	...
	public function searchPatientSLHCC()
	{		
		$data['param'] = $this->input->get('param'); //added by Mike, 20170616
		...
		$this->load->view('searchPatientSLHCC', $data);
	}

	//added by Mike, 20200517; edited by Mike, 20200704
	public function confirmPatientSLHCC()
	{
		//edited by Mike, 20200407
		$data['nameParam'] = $_POST['nameParam'];
		...
		//added by Mike, 20200328
		if (!isset($data['nameParam'])) {
			redirect('browseSLHCC/searchPatientSLHCC');
		}
		
		date_default_timezone_set('Asia/Hong_Kong');
		$dateTimeStamp = date('Y/m/d H:i:s');

		$this->load->model('BrowseSLHCC_Model');
		
		//edited by Mike, 20200602
		//$data['result'] = $this->Browse_Model->getDetailsListViaName($data);
		$data['result'] = $this->BrowseSLHCC_Model->getNewestPatientDetailsListViaName($data);
	...
	public function searchPatientInformationDeskSLHCC()
	{		
		...
		$this->load->view('searchPatientInformationDeskSLHCC', $data);
	}

	//added by Mike, 20200529; edited by Mike, 20200628
	public function confirmPatientInformationDeskSLHCC()
	{
		$data['nameParam'] = $_POST['nameParam'];
		
		//added by Mike, 20200328
		if (!isset($data['nameParam'])) {
			redirect('browseSLHCC/searchPatientInformationDeskSLHCC');
		}
		...
		$data['medicalDoctorList'] = $this->BrowseSLHCC_Model->getMedicalDoctorList();
...
*/	
	...
	public function viewPatientSLHCC($patientId)//PTRehab($patientId)
	{
//		$data['nameParam'] = $_POST[nameParam];
...
//		$data['medicalDoctorList'] = $this->BrowseSLHCC_Model->getMedicalDoctorList();
		$data['therapistList'] = $this->BrowseSLHCC_Model->getTherapistList();
		$data['result'] = $this->BrowseSLHCC_Model->getDetailsListViaId($patientId);
...
//		$data['resultPaid'] = $this->BrowseSLHCC_Model->getPaidPatientDetailsList($therapistId, $patientId);
//		$data['resultPaid'] = $this->BrowseSLHCC_Model->getPaidPatientDetailsListWithTreatment($therapistId, $patientId);
		$data['resultPaid'] = $this->BrowseSLHCC_Model->getPaidPatientDetailsListWithTreatment($data['therapistId'], $patientId);	
...
//		$this->load->view('viewPatientPTRehab', $data);
		$this->load->view('viewPatientSLHCC', $data);
	}
...
*/		
	...
	public function addTransactionServicePurchaseSLHCC($therapistId, $patientId, $scheduleDate, $scheduleTime, $diagnosis, $temperature, $bloodPressure)
	{		
		...
		$this->BrowseSLHCC_Model->addTransactionServicePurchaseSLHCC($data);
...
*/		
	...
	//added by Mike, 20200628
	public function addNewTransactionForPatientSLHCC($patientId, $medicalDoctorId)
	{		
		$data['patientId'] = $patientId;
		$data['medicalDoctorId'] = $medicalDoctorId;
		...
		$this->load->model('BrowseSLHCC_Model');
		$this->load->model('ReportSLHCC_Model');

		$this->BrowseSLHCC_Model->addNewTransactionForPatientPTRehab($data);

//		$data["result"] = $this->Report_Model->getPatientQueueReportForTheDay();
		//removed by Mike, 20200628
		$data["result"] = $this->ReportSLHCC_Model->getPatientPTRehabListOfAppointmentReportForTheDay();

		//edited by Mike, 20200601
		//this is so that we do not add excess transactions
		//$this->load->view('viewReportPatientQueue', $data);
//		redirect('report/viewReportPatientQueue');
		redirect('reportSLHCC/viewReportPTRehabListOfAppointment');	
	}
...
*/
	...
	//added by Mike, 20200529
	public function addPatientNameInformationDeskSLHCC()
	{
		$data['patientLastNameParam'] = $_POST['patientLastNameParam'];
		$data['patientFirstNameParam'] = $_POST['patientFirstNameParam'];
		...
		if (!isset($data['patientLastNameParam'])) {
			redirect('reportSLHCC/viewReportPatientAppointmentList');
		}
		...
		$patientId = $this->BrowseSLHCC_Model->addPatientName($data);
...
//		$this->addNewTransactionForPatient($patientId, $data['medicalDoctorIdParam']);
		$this->viewPatientSLHCC($patientId);
	}
...
//		$data['cartListResult'] = $this->Browse_Model->getItemDetailsListViaNotesUnpaid();
	...
	public function deleteTransactionFromPatientSLHCC($patientId, $transactionId)
	{
		$data['transactionId'] = $transactionId;
		...
		$this->BrowseSLHCC_Model->deleteTransactionFromPatientSLHCC($data);
