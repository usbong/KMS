<!--
  ...
  @date created: 20210808
  @date updated: 20210808
  @website address: www.usbong.ph

  Input:
  1) Laboratory Results Miscellaneous Form (.csv format) at the Marikina Orthopedic Specialty Clinic (MOSC)
  Output:
  1) Laboratory Results Miscellaneous Form that is viewable on a Computer Web Browser  
...
-->
						...
						td.bottomSectionRightSideColumn
						{
							width: 74%;
							text-align: right;
						}
...
//	$filename="D:\\Usbong\\LABORATORY\\templates\\MOSCLabRequestForm.csv";
	...
	//edited by Mike, 20210808
	//$filename="C:\\xampp\\htdocs\\usbong_kms\\usbongTemplates\\MOSCLabRequestForm.csv";
	
	$filename="C:\\xampp\\htdocs\\usbong_kms\\usbongTemplates\\MOSCLabResultsMiscForm.csv";
...
?>
	...
	<div class="contactDiv">
		<span>#2 E. MANALO (FORMERLY, E. RODRIGUEZ) AVE., STO. NIÑO, 1820, MARIKINA CITY, PHILIPPINES</span>
	<br />
		<span>TEL. NO. (8) 941-4888; MONDAY ~ SATURDAY; 9:00 A.M. ~ 4:00 P.M.</span>
	</div>

	<table class="formTable">
	  <tr>
		<td>
			<h3>LAB RESULTS: MISCELLANEOUS FORM</h3>
		</td>
...
*/					
						...
						if (strpos($cellValue,"?")!==false) {							
							//added by Mike, 20210808
							if (strpos($cellValue,"textbox")!==false) {
								
								$cellValue=str_replace("?textbox","",$cellValue);
								$cellValue="<input class='inputAnswerTextBox no-spin' type='text' name='inputTextBox".$iTextboxCount."'>".$cellValue;
							}
							else {
...
<?php
						...
						//added by Mike, 20210210
							...
							//added by Mike, 20210808						
//							if (($iRowCount>=3) and ($iColumnCount<=2)){
							if ($iRowCount>=3) {
								//blank space HTML command: "&nbsp;"
								echo "<td class='columnHeaderField'><b>".$cellValue."</b></td>";							
							}
...
?>
	...
	<table>
		<tr>
			...
			<td class="bottomSectionRightSideColumn">
				<b>PATHOLOGIST</b>
			</td>
		</tr>
		<tr>
			<td>
<?php
			$medicalTechnologistId = ""; //$value["medical_technologist_id"];
			if ($medicalTechnologistId=="") {
				$medicalTechnologistId=0; //default;
			}
	
			echo "<select class='medicalTechnologistSelect' id='medicalTechnologistIdParam' name='selectMedicalTechnologistNameParam' form='labResultsMiscFormId'>";
										
				//No member added in Medical Technologist table yet
				if (empty($medicalTechnologistList)) { 
					echo "<option class='medicalTechnologistOption' value=''>HALIMBAWA</option>";
				}
				else {
					foreach ($medicalTechnologistList as $medicalTechnologistValue) {
						  if (isset($medicalTechnologistId) and ($medicalTechnologistValue["medical_technologist_id"]==$medicalTechnologistId)) {
							echo "<option class='medicalTechnologistOption' value='".$medicalTechnologistValue["medical_technologist_id"]."' selected='selected'>".$medicalTechnologistValue["medical_technologist_name"]."</option>";
						  }			  	  
						  else {
							echo "<option class='medicalTechnologistOption' value='".$medicalTechnologistValue['medical_technologist_id']."'>".$medicalTechnologistValue["medical_technologist_name"]."</option>";			  
						  }				
					}
				}
			echo "</select>";
